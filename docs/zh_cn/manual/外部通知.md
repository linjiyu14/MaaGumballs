...
参照Qsmg的官方[文档地址](https://qmsg.zendee.cn/docs/)，将分配的机器人**添加qq好友**，获取自己的**key**，然后将自己的qq加入列表，如下图所示：
![qmsg管理台设置](https://gitee.com/dealos/image-upload/raw/master/MaaGumballs/qmsg3_bot.png)

> [!INFO]
> 如果自己有bot，也可以托管到qmsg上，点击[链接](https://qmsg.zendee.cn/bot)跳转配置，绑定bot后，同样需要将key发送给bot完成接收账号与bot的绑定，才能接收消息。

MFAAvalonia 配置：
| 选项                                                         | 值                                                           |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| Server | https://qmsg.zendee.cn/v3 (qmsg3.0版本接口后面带“v3”，如果是自建服务器请自行填写) |
| Key | 填写自己的key(上图第二步获取的key) |
| 用户QQ | 填写自己的qq(上图第三步填写的QQ) |
| 机器人QQ | 填写机器人的qq(上图第一步分配的机器人QQ，qmsg3.0版本可以不用填) |
